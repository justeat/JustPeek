#
# Be sure to run `pod lib lint JustPeek.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'JustPeek'
  s.version          = '0.1.0'
  s.summary          = 'iOS Library that adds support for Force Touch-like Peek and Pop interactions on older devices'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
iOS Library that adds support for Force Touch-like Peek and Pop interactions on older devices. It uses the native implementation if available, or a custom implementation otherwise
                       DESC

  s.homepage         = 'https://github.com/<GITHUB_USERNAME>/JustPeek'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'Gianluca Tranchedone' => 'g.tranchedone@gmail.com' }
  s.source           = { :git => 'https://github.com/justeat/JustPeek.git', :tag => s.version.to_s }
  s.social_media_url = 'https://twitter.com/gtranchedone'

  s.ios.deployment_target = '8.0'

  s.source_files = 'JustPeek/Classes/**/*'
  s.frameworks = 'UIKit'
end
